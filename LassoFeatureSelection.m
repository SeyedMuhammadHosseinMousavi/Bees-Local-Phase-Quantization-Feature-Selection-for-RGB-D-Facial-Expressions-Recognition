%% Lasso + LPQ Image Feature Selection Pipeline
...
[B, FitInfo] = lasso(x, t, 'Alpha', 1, 'CV', 5);  % Cross-validated Lasso

% Select non-zero coefficients (selected features)
lasso_features_idx = find(B(:, FitInfo.IndexMinMSE) ~= 0);
lasso_features = x(:, lasso_features_idx);

% Combine the selected features with labels
LPQ_Lasso_Features = [lasso_features, t];
...
X = LPQ_Lasso_Features(:, 1:end-1);  % Features
y = LPQ_Lasso_Features(:, end);      % Labels
